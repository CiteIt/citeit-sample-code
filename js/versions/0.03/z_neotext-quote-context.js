/*
 * CiteIt Quote-Context Javascript Library
 * https://github.com/neotext/neotext-quote-context/
 *
 * Copyright 2015-2018, Tim Langeman
 ...
 *
 * The CiteIt web service returns a json dictionary and this script
 * injects the returned contextual data into hidden html elements to be
 * displayed when the user hovers over or expands the cited quote.
 *
 * Demo: http://www.neotext.net/demo/
 *
 * Dependencies:
 *  - jQuery: https://jquery.com/
 *  - Sha256: https://github.com/brillout/forge-sha256/blob/master/build/forge-sha256.js
*/
...
/* Get Page's Canonical URL, stripping off anchor */
citeit_canonical_url = citeit_url();
...
jQuery.fn.quoteContext = function() {
	...
	jQuery(this).each(function(){
		...
		if( jQuery(this).attr("cite") ){
			...
			var citing_url = citeit_canonical_url;
			...
			if (cited_url.length > 3){
				...
				function add_quote_to_dom(tag_type, json ) {
					if ( tag_type == "q"){
						...
						blockcite.before(" \
							<div id='quote_before_" + json['sha256'] + "' class='quote_context'> \
...
function quote_hash(citing_quote, citing_url, cited_url){
	var url_quote_text = escape_quote(citing_quote) + '|' + escape_url(citing_url) + '|' + escape_url(cited_url);
	var quote_hash = forge_sha256(url_quote_text);
	alert(quote_hash);
	return quote_hash;
}
